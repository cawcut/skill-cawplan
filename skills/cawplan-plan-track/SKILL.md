...
description: |
  Track CawPlan release progress for a version: ticket completion, risk level, open items, target release dates, unresolved Critical-priority bugs, features not yet at QA Testing, descope suggestions, and what's blocking QA testing.
  Use when: the user asks to track a release, check release progress, review open tickets for a version, assess release/delay risk, ask what to descope, or ask what's blocking QA testing for a version.
...
7. **Descope suggestions** (only when the user explicitly asks what to descope or how to reduce delay risk — never suggest this unprompted):
...
- If step 7 ran: descope suggestions as a ranked list, each with the ticket and the one-line reason it's eligible (priority + status category) — not a restated risk_reason.
- If step 8 ran: which in-testing tickets are blocked, by what, and the blocker's own status — or state explicitly that no in-testing ticket has an unresolved blocker.
