...
- ASKING vs CHANGING (topic): when the human asks how/why/whether CURRENT behavior works ("有没有/是不是/怎么做的/会带哪些/看下/你打算怎么做", "这个需要改什么", "为什么浏览器") and requests no change, topic is investigation — even if the subject is CSP, API gateway strip_length, or WebSocket config. Use integration_api only when the human asks to wire/change API fields or simulate an API call; use security when implementing permission/CSP policy changes; use bug when reporting a broken connection/error/regression (including CSP-blocked WebSocket that fails in browser).
...
- pasted symptom narrative without a question ("clicked allow but dialog still appears") -> context_supply / security or bug (NOT question_clarification).
...
- symptom narrative only ("clicked allow but dialog still appears") -> context_supply / security (NOT correction_defect).
